...
class DaDuyetController extends GetxController with StateMixin<List<XetDuyet>> {
  ...
  final DaDuyetRepository _daDuyetRepository =
      DaDuyetRepository(provider: DaDuyetProviderAPI(AuthService()));
  ...
  void fetchListContent({bool isLoadMore = false}) async {
    ...
    final result = await _daDuyetRepository.getDaDuyet(
      ma: ma,
    );
