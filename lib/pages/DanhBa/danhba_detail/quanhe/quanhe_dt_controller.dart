...
import 'package:salesoft_hrm/API/repository/danhba_dt/quanhe_dt_repository.dart';
import 'package:salesoft_hrm/model/quanhe_model.dart';

class QuanHeDtController extends GetxController with StateMixin<Quanhe> {
  ...
  final QuanHeDtRepository _quanheRepository =
      QuanHeDtRepository(provider: QuanHeDtProviderAPI());
  ...
  void fetchListContent({bool isLoadMore = false}) async {
    ...
    final result = await _quanheRepository.getQuanHeDt(
      pageSize: pageSize,
      pageIndex: pageIndex,
      ma: ma,
    );
